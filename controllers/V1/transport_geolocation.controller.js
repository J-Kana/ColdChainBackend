...
const TransportGeo = db.transport_geolocation;

exports.getObject = (req, res) => {
    try {
        let {id} = req.params
        if(!id) {
            TransportGeo.findAll()
                .then(async objects => {
...
exports.objectUpdate = (req, res) => {
    try {
        let {id} = req.params
        let {body} = req
        TransportGeo.update(body, {where: {id: id}})
            .then((response) => {
...
exports.objectDelete = (req, res) => {
    try {
        let {id} = req.params
        TransportGeo.destroy({where: {id: id}})
            .then((response) => {
