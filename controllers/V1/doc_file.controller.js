...
exports.getObject = (req, res) => {
    try {
        ...
        if(!id) {
            Doc_File.findAll({where: {companyId: company}})
                .then(async objects => {
            ...
            Doc_File.findOne({where: {[Op.and]: {id: id, companyId: company}}})
                .then(async object => {
...
exports.objectUpdate = (req, res) => {
    try {
        let {id} = req.params
        let {body} = req
        Doc_File.update(body, {where: {id: id}})
            .then((response) => {
